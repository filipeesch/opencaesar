...
- **Typecheck sequencing note (plan-checker W1/W2):** the Wave-0 scaffolds and early-wave gates reference APIs that land in later waves (`src/game/advisors` → Wave 2; `SimRunner.getWaterOverlay`/`getInspector` → Waves 3/4). Consequently `npm run typecheck` in `<automated>` is RED at Waves 0-3 by design. Executor MUST handle this: for Wave 0, restrict typecheck to the src tree only (`npx tsc --noEmit src` or file-existence + targeted vitest on compiled-on-the-fly) and/or add `@ts-expect-error`/type-only stubs so the written scaffolds parse; Wave 1-3 per-wave gates may exclude `npm run typecheck` until Wave 4 (declare per-wave typecheck red-but-expected); the FINAL Wave-5 gate is authoritative (full typecheck green). The Wave-2 composer's housing panel reads `getWaterOverlay` — feature-detect/guard that read in `advisorPanels` (or read it lazily) so the composer typechecks before the getter lands in Wave 3.
